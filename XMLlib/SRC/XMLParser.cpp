...
namespace XML
{
	...
	XML::XMLDocument* XMLParser::ParseXML(char *Buffer, int BufferLenght)
	{
		if (namespacevector)
			DestoryVector<NameSpaceVector>(namespacevector);
		...
		{
			Error = 0;
			buffer = new UTF8Buffer(Buffer, BufferLenght);
			if (!buffer->ReadBOM())
			{
				Error = 1;
				ErrorList.push_back(L"Text is not UTF8 encoding");
				return NULL;
			}
	...
	XMLProlog*  XMLParser::ParseProlog()
	{
		...
		if (strncmp(prologBegin, "<?xml", 5) == 0)
		{
			...
			{
				Error = 3;
				ErrorList.push_back(L"Error get end of XML prolog");
				buffer->End();
			}
			else
			{
				...
				{
					Error = 3;
					ErrorList.push_back(L"Error get attrubutes in XML prolog");
					buffer->End();
				}
	...
	void XMLParser::CheckProlog(XMLProlog *prolog)
	{
		...
		{
			Error = 3;
			ErrorList.push_back(L"Error get attrubutes in XML prolog: Count of attibutes is wrong");
		}
		...
		for(unsigned int i = 0 ; i < prolog->attributes->size(); ++i)
		{
			...
			auto ch = str.c_str();
			if (wcscmp(ch, L"version") == 0)
				found_version = true;
			if (wcscmp(ch, L"encoding") == 0)
				found_encoding = true;
		}
	...
	AttibuteVector* XMLParser::ParseAttribute(char *Buffer, int BufferLenght)
	{
		UTF8Buffer attributebuf(Buffer, BufferLenght);
		...
		while (!attributebuf.IsEnd())
		{
			...
			if (!Name)
			{
				...
				Error = 2;
				ErrorList.push_back(L"Error parse xml attribute name");
				return NULL;
			}
			...
			if (!Value)
			{
				DestoryVector<AttibuteVector>(attributes);
				Error = 2;
				ErrorList.push_back(L"Error parse xml attribute value");
				delete Name;
				return NULL;
			}
	...
	XMLString* XMLParser::ParseAttributName(UTF8Buffer &buffer)
	{
		...
		{
			Error = 6;
			ErrorList.push_back(L"Error parse attribute name: Don't find '='");
			return NULL;
		}
		...
		delete Name;
		return new XMLString(TrimName, LenghtName);
	}

	XMLString * XMLParser::ParseAttributValue(UTF8Buffer &buffer, char &Quote)
	{
		...
		{
			Error = 76;
			ErrorList.push_back(L"Error parse attribute: Don't find end quote");
			return NULL;
		}
		...
		buffer.MoveNext(1);
		return new XMLString(Value,LenghtValue);
...
//			if (TAG[0] != '\0')
					...
					if (strncmp(TAG, "<!--", 4) == 0)
					{
						...
						if (strncmp(TAG, "</", 2) == 0)
						{
							...
							if (strncmp(TAG, "<", 1) == 0)
							{
								...
								{
									Error = 6;
									ErrorList.push_back(L"Error parse XML: Error in node");
									buffer->End();
								}
								else
									if (_parentNode != NULL)
							...
							else
							{
								...
								{
									Error = 5;
									ErrorList.push_back(L"Error parse XML: Don't find xml text in root document");
									buffer->End();
								}
								else
								{
									...
									its->indexChildText = -1;
									_parentNode->inner_text_list->push_back(its);
									DEBUG_OUT_INNER(_parentNode->inner_text);
								}
							}
			}
			delete TAG;
	...
	XMLNode * XMLParser::ParseXMLNodeName(XMLNode * _parentNode)
	{
		...
		while (!buffer->IsEnd())
		{
			...
			{
				Error = 6;
				ErrorList.push_back(L"Error parse XML: Unexpected char < or &");
				buffer->End();
				return NULL;
			}
		}
		...
		{
			Error = 6;
			ErrorList.push_back(L"Error parse XML: Don't find end node name");
			buffer->End();
			return NULL;
		}
		...
		XMLString *node_name = new XMLString(node_name_char, lenght);
		...
		DEBUG_OUT_TREE(L"<" + node->nameFull + L">");
	...
	bool   XMLParser::ParseXMLComment()
	{
		...
		{
			Error = 5;
			ErrorList.push_back(L"Error parse XML: Don't find end comment");
			buffer->End();
			return false;
		}
	...
	std::wstring XMLParser::ParseXMLInnerText()
	{
		...
		{
			Error = 8;
			ErrorList.push_back(L"Error parse end inner text: Don't find <");
			return NULL;
		}
	...
	std::wstring XMLParser::ParseXMLEndNode()
	{
		...
		{
			Error = 7;
			ErrorList.push_back(L"Error parse node end name: Don't find >");
			return NULL;
		}
	...
	{
		if (_text.getLenght() < 2)
			return true;
		
		const char *text = _text.getText();
		int position = -1;
		for (int i = 0; i < _text.getLenght(); i++)
		{
			if (text[i] == ':')
			{
				position = i;
